...
void gf3d_mesh_render(uint32_t entity_id, uint32_t swap_chain_image_id, Mesh *mesh,VkCommandBuffer commandBuffer, VkDescriptorSet * descriptorSet)
{
    ...
    gf3d_ubo_manager *const instance_ubo_manger = gf3d_vgraphics_get_instance_uniform_buffer_manager(),
                     *const global_ubo_manger = gf3d_vgraphics_get_global_uniform_buffer_manager();

    uint32_t dynamic_offsets[2] = {
            gf3d_uniforms_reference_offset_get(instance_ubo_manger, entity_id, swap_chain_image_id),
            gf3d_uniforms_reference_offset_get(global_ubo_manger, 0, swap_chain_image_id)
    };
...
Mesh *gf3d_mesh_load(char *filename)
{
    ...
    mesh->dimensions.x_min = mesh->dimensions.x_max = *point[0];
    mesh->dimensions.y_min = mesh->dimensions.y_max = *point[1];
    mesh->dimensions.z_min = mesh->dimensions.z_max = *point[2];
    ...
    for (Uint32 i = 0; i < obj->face_vert_count; i++) {
        ...
        /// X Axis
        if (x < mesh->dimensions.x_min)
            mesh->dimensions.x_min = x;

        if (x > mesh->dimensions.x_max)
            mesh->dimensions.x_max = x;

        /// Y Axis
        if (y < mesh->dimensions.y_min)
            mesh->dimensions.y_min = y;

        if (y > mesh->dimensions.y_max)
            mesh->dimensions.y_max = y;

        /// Z Axis
        if (z < mesh->dimensions.z_min)
            mesh->dimensions.z_min = z;

        if (z > mesh->dimensions.z_max)
            mesh->dimensions.z_max = z;
    }
